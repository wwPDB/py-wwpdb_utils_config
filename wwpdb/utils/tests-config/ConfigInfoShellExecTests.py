...
class ConfigDataSetExecTests(unittest.TestCase):
    ...
    def testBrokenConfig(self):
        ...
        ConfigInfoShellExec(topConfigPath="/tmp/ab12s", siteId="WWPDB_DEPLOY_TEST", siteLoc="rcsb-east", verbose=True, log=sys.stdout)

    def testHostnameLookup(self):
        """Tests locating by host name - no cache"""
        tcise = ConfigInfoShellExec(siteId="WWPDB_DEPLOY_TEST", hostName="testhost.test.com", verbose=True, cacheFlag=False, log=sys.stdout)
        tcise.shellConfig()
